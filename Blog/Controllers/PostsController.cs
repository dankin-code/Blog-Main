...
namespace Blog.Controllers
{
    public class PostsController : Controller
    {
        ...
        public ActionResult Create(Post post, HttpPostedFileBase MediaUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                //if (ImageUploadValidator.IsWebFriendlyImage(MediaUrl))
                if (MediaUrl.ContentLength > 0)
                {
                    var fileName = Path.GetFileName(MediaUrl.FileName);
                    MediaUrl.SaveAs(Path.Combine(Server.MapPath("~/Content/Images/"), fileName));
                    post.MediaUrl = "~/Content/Images/" + fileName;
                }
